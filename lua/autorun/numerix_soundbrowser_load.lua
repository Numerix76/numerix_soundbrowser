...
SoundBrowser = SoundBrowser or {}

SoundBrowser.Settings = SoundBrowser.Settings or {}
SoundBrowser.Language = SoundBrowser.Language or {}

local FileSystem = "soundbrowser"
local AddonName = "SoundBrowser"
local Version = "1.0.2"
...
if SERVER then
    ...
    if FromWorshop then
        if SoundBrowser.Settings.VersionDefault != SoundBrowser.Settings.VersionCustom then
            hook.Add("PlayerInitialSpawn", "SoundBrowser:PlayerInitialSpawnCheckVersionConfig", function(ply)
                if ply:IsSuperAdmin() then
                    timer.Simple(10, function()
                        ply:SoundBrowserChatInfo(SoundBrowser.GetLanguage("A new version of the config file is available. Please download it."), 1)
                    end)
                end
            end)
        end

        if SoundBrowser.Language.VersionDefault != SoundBrowser.Language.VersionCustom then
            hook.Add("PlayerInitialSpawn", "SoundBrowser:PlayerInitialSpawnCheckVersionLanguage", function(ply)
                if ply:IsSuperAdmin() then
                    timer.Simple(10, function()
                        ply:SoundBrowserChatInfo(SoundBrowser.GetLanguage("A new version of the language file is available. Please download it."), 1)
                    end)
                end
            end)
        end
    ...
    hook.Add("PlayerConnect", "SoundBrowser:Connect", function()
            ...
            http.Fetch( "https://gmod-radio-numerix.mtxserv.com/api/version/"..FileSystem..".txt",
                function( body, len, headers, code )
                    if body != Version then
                        hook.Add("PlayerInitialSpawn", "SoundBrowser:PlayerInitialSpawnCheckVersionAddon", function(ply)
                            if ply:IsSuperAdmin() then
                                timer.Simple(10, function()
                                    ply:SoundBrowserChatInfo(SoundBrowser.GetLanguage("A new version of the addon is available. Please download it."), 1)
                                end)
                            end
                        end)
                    end 
        ...
        hook.Remove("PlayerConnect", "SoundBrowser:Connect")
    end)

    hook.Add("ShutDown", "SoundBrowser:Disconnect", function()
        if !game.SinglePlayer() then
